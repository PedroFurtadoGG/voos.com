...
public class GeraHTML {
    ...
    public String getListCidade(List lista){
        ...
        for (Iterator it = lista.iterator(); it.hasNext();) {
            ...
            sb.append("<li>"
                    ...
                    + "<p class='booking-item-car-title'>Quantidade poltronas:" + c.getUf()+ "</p>"
                    + "</div>"
                    + "</div>"
                    + "<div class='col-md-6'>"
                    + "<div clas='row'>"
    ...
    public String getFormAtualizarCidade(Cidades c) {
        ...
        sb.append("\t\t<h4>Informações da Cidade</h4>"
                ...
                + "<div class='form-group'>"
                + "<label>UF</label>"
                + "<select name='uf'><option value='AC'>Acre</option><option value='AL'>Alagoas</option><option value='AP'>Amapá</option><option value='AM'>Amazonas</option><option value='BA'>Bahia</option><option value='CE'>Ceará</option><option value='DF'>Distrito Federal</option><option value='ES'>Espírito Santo</option><option value='GO'>Goiás</option><option value='MA'>Maranhão</option><option value='MT'>Mato Grosso</option><option value='MS'>Mato Grosso do Sul</option><option value='MG'>Minas Gerais</option><option value='PA'>Pará</option><option value='PB'>Paraíba</option><option value='PR'>Paraná</option><option value='PE'>Pernambuco</option><option value='PI'>Piauí</option><option value='RJ'>Rio de Janeiro</option><option value='RN'>Rio Grande do Norte</option><option value='RS'>Rio Grande do Sul</option><option value='RO'>Rondônia</option><option value='RR'>Roraima</option><option value='SC'>Santa Catarina</option><option value='SP'>São Paulo</option><option value='SE'>Sergipe</option><option value='TO'>Tocantins</option></select>"
                + "</div>"
                + "<hr>"
                + "<input type='submit' class='btn btn-primary' value='Salvar'>");
        sb.append("\t</form>\r\n");
